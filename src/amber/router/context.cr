...
class HTTP::Server::Context
  ...
  def cookies
    @cookies ||= Amber::Router::Cookies::Store.build(request, Amber::Server.settings.secret_key_base)
  end

  def session
    @session ||= Amber::Router::Session::Store.new(cookies, Amber::Server.settings.session).build
  end
