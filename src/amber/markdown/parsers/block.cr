module Amber::Markdown::Parser
  class Block
    ...
    private def process_task_lists
      walker = @document.walker
      while (event = walker.next)
        node, entering = event
        next unless entering && node.type.item?

        # Check the first child - for paragraphs, the text content is stored
        # in the paragraph's text property before inline parsing
        first_child = node.first_child?
        next unless first_child

        # For tight lists, the content may be in a paragraph child
