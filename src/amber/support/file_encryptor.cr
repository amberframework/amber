...
module Amber::Support
  ...
  module FileEncryptor
    def read(filename : String, encryption_key = encryption_key)
    ...
    def write(filename : String, body : (String | Bytes), encryption_key = encryption_key)
      encryptor = MessageEncryptor.new(encryption_key)
      File.write(filename, encryptor.encrypt_and_sign(body))
    end

    def read_as_string(filename, encryption_key = encryption_key)
