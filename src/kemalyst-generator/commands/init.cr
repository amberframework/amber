...
module Kemalyst::Generator
  ...
  class MainCommand < Cli::Supercommand
    ...
    class Init < Cli::Command
      class Options
        arg "type", desc: "Project type: app, spa, api", required: true
        arg "name", desc: "Name of application", required: true
      end

      def run
        init = Application.new(args.name, "./#{args.name}")
        init.build_project args.type
      end
    end
  end
  ...
  class Application
    ...
    def initialize(name : String, directory : String)
      ...
      if Dir.exists?(@directory)
        raise "Directory already exists: #{@directory}"
      else
