...
describe Amber::CMD do
  context "Init command" do
    it "should create the new project" do
      Amber::CMD::MainCommand.run ["init", "app", TESTING_APP]
    ...
    it "should create the structure" do
      ...
      File.read_lines("#{TESTING_APP}/shard.yml")[18]?.should eq "  pg:"
      `rm -rf #{TESTING_APP}`
    end

    it "should create app with mysql settings" do
      Amber::CMD::MainCommand.run ["init", "app", TESTING_APP, "-d", "mysql"]
      File.read_lines("#{TESTING_APP}/config/database.yml").first.should eq "mysql:"
      File.read_lines("#{TESTING_APP}/shard.yml")[18]?.should eq "  mysql:"
      `rm -rf #{TESTING_APP}`
    end

    it "should create app with sqlite settings" do
      Amber::CMD::MainCommand.run ["init", "app", TESTING_APP, "-d", "sqlite"]
      File.read_lines("#{TESTING_APP}/config/database.yml").first.should eq "sqlite:"
      File.read_lines("#{TESTING_APP}/shard.yml")[18]?.should eq "  sqlite3:"
      `rm -rf #{TESTING_APP}`
    end

    it "should generate .amber.yml with language settings" do
      Amber::CMD::MainCommand.run ["init", "app", TESTING_APP, "-t", "ecr"]
