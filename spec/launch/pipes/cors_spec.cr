...
module Launch::Pipe
  describe CORS do
    ...
    it "supports expose header configuration" do
      ...
      context.response.headers[Launch::Pipe::Headers::ALLOW_EXPOSE].should eq expose_header.join(",")
    end
    ...
    it "adds vary header when origin is other than (*)" do
      ...
      cors_add_next(CORS.new(origins: origins)).call(context)
      context.response.headers[Launch::Pipe::Headers::VARY].should eq "Origin"
    end
    ...
    it "does not add vary header when origin is (*)" do
      ...
      cors_add_next(CORS.new(origins: origins)).call(context)
      context.response.headers[Launch::Pipe::Headers::VARY]?.should be_nil
    end

    it "adds Vary header based on :vary option" do
      context = cors_context("GET", "Origin": "example.com")
      cors_add_next(CORS.new(origins: origins, vary: "Other")).call(context)
      context.response.headers[Launch::Pipe::Headers::VARY].should eq "Origin,Other"
    end
    ...
    it "sets allow credential headers if credential settings is true" do
      ...
      context.response.headers[Launch::Pipe::Headers::ALLOW_CREDENTIALS].should eq "true"
    end
